...
import br.com.onesystem.domain.builder.CambioEmpresaBuilder;
...
public class CambioEmpresaBV implements BuilderView<CambioEmpresa>, Serializable {
    ...
    public CambioEmpresaBV() {
    }

    public CambioEmpresaBV(CambioEmpresa dp) {
        this.id = dp.getId();
    ...
    public CambioEmpresa construir() throws DadoInvalidoException {
        return new CambioEmpresaBuilder().comDestino(destino).comOrigem(origem).comValor(valor)
                .comValorConvertido(valorConvertido).comBaixas(baixas).comEmissao(emissao).construir();
    }

    @Override
    public CambioEmpresa construirComID() throws DadoInvalidoException {
        return new CambioEmpresaBuilder().comId(id).comDestino(destino).comOrigem(origem).
